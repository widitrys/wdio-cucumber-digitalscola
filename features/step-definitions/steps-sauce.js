...
When (/^user clik button add to cart$/, async () => {
...
Then(/^user is on dashboard page$/, async () => {
    const pageText = await browser.$('body');
    expect(pageText).toHaveTextContaining('Sauce Labs Bike Light')
});

Then(/^user see product on cart page$/, async () => {
    const pageText = await browser.$('body');
    expect(pageText).toHaveTextContaining('Sauce Labs Bike Light')
});
